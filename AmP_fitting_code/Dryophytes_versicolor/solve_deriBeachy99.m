...
function [tout, Xout, par, phys, TE, YE, IE] = solve_deriBeachy99(t, X0, par)
    ...
    % set up the ODE solver settings
    options = odeset(Events=@reach_climax, RelTol=1e-5, AbsTol=1e-5);
    [tout_tmp1,Xout_tmp1,TE,YE,IE] = ode45(@derivativesBeachy99,t,X0,options,par);
    ...
    if size(t,1)==1
        t2 = [0 (t(t>tout_tmp1(end))-tout_tmp1(end))];
    else
        t2 = [0; (t(t>tout_tmp1(end))-tout_tmp1(end))]';
    end 
    X0 = Xout_tmp1(end,:);
    ...
    [tout_tmp2,Xout_tmp2,TE2,YE2,IE2] = ode45(@derivatives_testBeachy99,t2,X0,options,par);
    if length(t2)==2
    ...
    if par.terminate == 1
        if (size(t,1)==1)
         ...
         Xout = [vertcat(Xout_tmp1(1:end-1,:), Xout_tmp2(2:end,1:6)) ; repelem([10000 10000 10000 10000 10000 10000],length(t(t>(tout_tmp2(end) + tout_tmp1(end-1)) )),1)];
    end
