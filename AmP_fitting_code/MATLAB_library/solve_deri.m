...
function [tout, Xout, par, phys, TE, YE, IE] = solve_deri(t, X0, par)
    ...
    [tout_tmp1,Xout_tmp1,TE,YE,IE] = ode45(@derivatives,t,X0,options,par);
    %FT = arrhenius(Texp, Tref, par.TA, Tref_l, TAl, Tref_h, TAh);

    Xout_tmp = Xout_tmp1; 
    
    % make sure to return unreasonable values in case not even one of the
    % events can be reached (needed for the use in a fitting rountine)
    if isempty(YE)
        tout = tout_tmp1;
        Xout = Xout_tmp1;
        TE(3:4)=[100 100];
        ...
        IE(3)=3;
        IE(4)=4;
        e = Xout(:,1) ./ (E_M * (Xout_tmp(:,3).^3)) ;
        phys.e =e;
        return;
    elseif (YE(end,2)<par.EH42)  % cannot reach start of the climax
        tout = tout_tmp1;
        Xout = Xout_tmp1;
        TE(3:4)=[100 100];
        YE(3:4,:) = repelem([10000 10000 10000 10000 10000 10000],2,1);
    ...
    if par.terminate==2 
         ...
         if (size(t,1)==1)
             ...
             Xout = [Xout_tmp1(1:end-1,:) ; t(t>tout_tmp1(end))'.*repelem([100 100 100 100 100 100],length(t(t>tout_tmp1(end))),1)];
         else
             tout = [tout_tmp1(1:end-1); t(t>tout_tmp1(end))];
             Xout = [Xout_tmp1(1:end-1,:) ; t(t>tout_tmp1(end)).*repelem([100 100 100 100 100 100],length(t(t>tout_tmp1(end))),1)];
         end
         % fill all the quantities that need to be returned
    ...
    X0 = Xout_tmp1(end,:);
    % par.Lj = X0(3);  % length of the tadpole at the start of the metamorphosis climax
    % X0(7) = par.f;
    % X0(7) = 0;
    % X0(8) = X0(3)^3 * (1-par.reprfrac); 
    % X0(9) = X0(3)^3 * (par.reprfrac);
    % par.pLfix = X0(9) * par.v / par.Lj;

    % solve the equations during the metamorphosis climax
    options = odeset(Events=@end_metamorph, RelTol=1e-5, AbsTol=1e-5);
    [tout_tmp2,Xout_tmp2,TE2,YE2,IE2] = ode45(@derivatives_test,t,X0,options,par);
...
% event function for the end of hte metamorphosis and reach of puberty
    EHj  = par.EHj;
    EHp  = par.EHp;
    nevents = 2;
    value = zeros(nevents,1);    
    value(1) = X(2) - EHj; % end climax
    value(2) = X(2) - EHp; % puberty
...
% event function for developmental milestones before the end of the climax
    EHh  = par.EHh; % hatch
    EHb  = par.EHb; % birth
    EH42 = par.EH42; % start climax
    nevents = 3;
    value = zeros(nevents,1);    
    value(1) = X(2) - EHh;
    value(2) = X(2) - EHb;
